...
void LU(matrix<T> & A,matrix<T> * L,matrix<T> * U)
{
   *U = A;
        ...
        #pragma omp parallel for 
	for(int i = 0; i <= A.x_count; i++)
        {
		for(int j = i; j <= A.x_count; j++)
			L->matr2[j][i]=(*U)[j][i]/(*U)[i][i];
        }

       
	for(int k = 1; k <= A.x_count; k++)
	{
                #pragma omp parallel for
		for(int i = k-1; i <= A.x_count; i++)
			for(int j = i; j <= A.x_count; j++)
				L->matr2[j][i]=(*U)[j][i]/(*U)[i][i];
           
                 #pragma omp parallel for
		for(int i = k; i <= A.x_count; i++)
			for(int j = k-1; j <= A.x_count; j++)
				U->matr2[i][j]=(*U)[i][j]-(*L)[i][k-1]*(*U)[k-1][j];
	}
}
//----------------------------------------------
...
//---------------------------------------------
vector<T> solve_R_B_eq(matrix<T> * R,vector<T> B)
{
  int n =R->x_count;
...
//-------------------------------------------------------
void createIakobi(Feug_function<T> & G,int N,vector<T> & X,vector<T> & BUF1,vector<T> & BUF2)
{
   size_t i,j;

   vector <T> buf3(N+1,0);
   vector <T> buf2_pow(N+1,1);
   vector <T> buf3_pow(N+1,1);
   vector <T> buf4_pow(N+1,1);
    ...
    for (i = 0; i<= (size_t) x_count ; i++)
    {
      //  buf1[i] = G.g(X[i]/G.alfa);
        buf3[i] =  G.diff_g( BUF1[i] );

        matr2[i][0] = buf3[i] * G.diff_g(X[i]/G.alfa) * (-X[i]/G.alfa)  + BUF2[i]  ;
       
       
        for (j = 1; j<= (size_t) y_count ; j++)
        {
          buf2_pow[i] *= BUF1[i]*BUF1[i];
          buf3_pow[i] *= X[i]*X[i];
          buf4_pow[i] *= ( X[i]*X[i]  / (G.alfa * G.alfa ) );          

          matr2[i][j] = G.alfa*buf2_pow[i] +  buf3_pow[i] + G.alfa* buf3[i] * buf4_pow[i] ;
