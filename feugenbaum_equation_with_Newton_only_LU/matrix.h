...
void init_LU(matrix<T> & A)
{
   ...
   LU(A,L,R);//по другому уже тут работает
...
void createIakobi(Feug_function<T> & G,int N,vector<T> & X,vector<T> & BUF1,vector<T> & BUF2)
{
   ...
   vector <scalar> * BUF5_pow = new vector<scalar>(N+1,1);
    ...
    for (i = 0; i<= (size_t) x_count ; i++)
    {
          ...
          buf4[i] = 0;
          (*BUF5_pow)[i] = X[i] / G.alfa ;
         for (int j = 1;j <= G.N ; j ++ )
          {
              buf3[i] += (2*j) * G.a[j]* (*BUF4_pow)[i];
              (*BUF4_pow)[i] *= ( BUF1[i] * BUF1[i] );

              buf4[i] += (2*j) * G.a[j]* (*BUF5_pow)[i];
              (*BUF5_pow)[i] *= ( X[i]*X[i]  / (G.alfa * G.alfa ) );
         }

        matr2[i][0] = buf3[i] * buf4[i] * (-X[i]/G.alfa)  + BUF2[i]  ;
        ...
        for (j = 1; j<= (size_t) y_count ; j++)
        {
          ...
          matr2[i][j] = G.alfa*(*BUF2_pow)[i] + (*BUF3_pow)[i] + G.alfa* buf3[i] * (*BUF1_pow)[i] ;
